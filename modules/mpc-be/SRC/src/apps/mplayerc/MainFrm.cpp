	...
	while (m_pME && SUCCEEDED(m_pME->GetEvent(&evCode, &evParam1, &evParam2, 0))) {
		...
		switch (evCode) {
			...
			case EC_DVD_PLAYBACK_RATE_CHANGE:
				if (m_pDVDC) {
					if (m_fCustomGraph && s.fullScreenModes.bEnabled == 1 &&
...
CString CMainFrame::OpenCapture(OpenDeviceData* pODD)
{
	...
	if (pVidCapTmp) {
		...
		if (pAMTuner) { // load saved channel
			...
			int country = 1;
			profile.ReadInt(L"Capture", L"Country", country);
			pAMTuner->put_CountryCode(country);

			int vchannel = pODD->vchannel;
			if (vchannel < 0) {
				profile.ReadInt(L"Capture\\" + CString(m_VidDispName), L"Channel", vchannel);
			}
			if (vchannel >= 0) {
				OAFilterState fs = State_Stopped;
				m_pMC->GetState(0, &fs);
