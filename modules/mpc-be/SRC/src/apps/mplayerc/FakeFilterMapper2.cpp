	...
	*/
	if (!pUnkOuter)
		if (rclsid == CLSID_VideoMixingRenderer || rclsid == CLSID_VideoMixingRenderer9
			...
			HRESULT hr;
			if (SUCCEEDED(hr = Real_CoCreateInstance(rclsid, pUnk, dwClsContext, IID_PPV_ARGS(&pInner)))) {
				pMK->SetInner(pInner);
				return pUnk->QueryInterface(riid, ppv);
			}
		}
...
void CFilterMapper2::UnInit()
{
	if (fInitialized) 
	{
		DetourTransactionBegin();
		DetourUpdateThread(GetCurrentThread());
		DetourDetach(&(PVOID&)Real_CoCreateInstance, (PVOID)Mine_CoCreateInstance);
		DetourDetach(&(PVOID&)Real_RegCloseKey, (PVOID)Mine_RegCloseKey);
		DetourDetach(&(PVOID&)Real_RegFlushKey, (PVOID)Mine_RegFlushKey);
		DetourDetach(&(PVOID&)Real_RegCreateKeyA, (PVOID)Mine_RegCreateKeyA);
		DetourDetach(&(PVOID&)Real_RegCreateKeyW, (PVOID)Mine_RegCreateKeyW);
		DetourDetach(&(PVOID&)Real_RegCreateKeyExA, (PVOID)Mine_RegCreateKeyExA);
		DetourDetach(&(PVOID&)Real_RegCreateKeyExW, (PVOID)Mine_RegCreateKeyExW);
		DetourDetach(&(PVOID&)Real_RegDeleteKeyA, (PVOID)Mine_RegDeleteKeyA);
		DetourDetach(&(PVOID&)Real_RegDeleteKeyW, (PVOID)Mine_RegDeleteKeyW);
		DetourDetach(&(PVOID&)Real_RegDeleteValueA, (PVOID)Mine_RegDeleteValueA);
		DetourDetach(&(PVOID&)Real_RegDeleteValueW, (PVOID)Mine_RegDeleteValueW);
		DetourDetach(&(PVOID&)Real_RegEnumKeyExA, (PVOID)Mine_RegEnumKeyExA);
		DetourDetach(&(PVOID&)Real_RegEnumKeyExW, (PVOID)Mine_RegEnumKeyExW);
		DetourDetach(&(PVOID&)Real_RegEnumValueA, (PVOID)Mine_RegEnumValueA);
		DetourDetach(&(PVOID&)Real_RegEnumValueW, (PVOID)Mine_RegEnumValueW);
		DetourDetach(&(PVOID&)Real_RegOpenKeyA, (PVOID)Mine_RegOpenKeyA);
		DetourDetach(&(PVOID&)Real_RegOpenKeyW, (PVOID)Mine_RegOpenKeyW);
		DetourDetach(&(PVOID&)Real_RegOpenKeyExA, (PVOID)Mine_RegOpenKeyExA);
		DetourDetach(&(PVOID&)Real_RegOpenKeyExW, (PVOID)Mine_RegOpenKeyExW);
		DetourDetach(&(PVOID&)Real_RegQueryInfoKeyA, (PVOID)Mine_RegQueryInfoKeyA);
		DetourDetach(&(PVOID&)Real_RegQueryInfoKeyW, (PVOID)Mine_RegQueryInfoKeyW);
		DetourDetach(&(PVOID&)Real_RegQueryValueA, (PVOID)Mine_RegQueryValueA);
		DetourDetach(&(PVOID&)Real_RegQueryValueW, (PVOID)Mine_RegQueryValueW);
		DetourDetach(&(PVOID&)Real_RegQueryValueExA, (PVOID)Mine_RegQueryValueExA);
		DetourDetach(&(PVOID&)Real_RegQueryValueExW, (PVOID)Mine_RegQueryValueExW);
		DetourDetach(&(PVOID&)Real_RegSetValueA, (PVOID)Mine_RegSetValueA);
		DetourDetach(&(PVOID&)Real_RegSetValueW, (PVOID)Mine_RegSetValueW);
		DetourDetach(&(PVOID&)Real_RegSetValueExA, (PVOID)Mine_RegSetValueExA);
		DetourDetach(&(PVOID&)Real_RegSetValueExW, (PVOID)Mine_RegSetValueExW);
	...
	, m_fRefCounted(fRefCounted), m_fAllowUnreg(fAllowUnreg)
{
	...
	HRESULT hr = Real_CoCreateInstance(CLSID_FilterMapper2, (IUnknown*)(INonDelegatingUnknown*)this, CLSCTX_ALL, IID_PPV_ARGS(&m_pFM2));
	if (FAILED(hr) || !m_pFM2) {
		ASSERT(0);
		return;
	}
