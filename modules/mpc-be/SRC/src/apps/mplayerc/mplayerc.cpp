...
extern "C" __declspec(dllexport) void db_ShowDllForm_Plugins(TLangStyle* lsFileType, char** strParentName, char** strSubModuleName, char** strClassName, char** strWindowName, const bool show = false)
{
    ...
    * strParentName    = "多媒体工具";  // 父模块名称
    * strSubModuleName = "MPC-BE(DLL)";// 子模块名称
    * strClassName     = "MPC-BE";     // 窗体类名
    * strWindowName    = "MPC-BE";     // 窗体名
	g_bInit = show;
...
BOOL CMPlayerCApp::InitInstance()
{
	...
	pFrame->ShowWindow((m_s.nCLSwitches & CLSW_MINIMIZED) ? SW_SHOWMINIMIZED : SW_HIDE);
	//pFrame->UpdateWindow();
...
void DropDetourHOOK()
{
	DetourTransactionBegin();
	DetourUpdateThread(GetCurrentThread());
	DetourDetach(&(PVOID&)Real_IsDebuggerPresent, (PVOID)Mine_IsDebuggerPresent);
	DetourDetach(&(PVOID&)Real_ChangeDisplaySettingsExA, (PVOID)Mine_ChangeDisplaySettingsExA);
	DetourDetach(&(PVOID&)Real_ChangeDisplaySettingsExW, (PVOID)Mine_ChangeDisplaySettingsExW);
	DetourDetach(&(PVOID&)Real_CreateFileA, (PVOID)Mine_CreateFileA);
	DetourDetach(&(PVOID&)Real_CreateFileW, (PVOID)Mine_CreateFileW);
	DetourDetach(&(PVOID&)Real_mixerSetControlDetails, (PVOID)Mine_mixerSetControlDetails);
	DetourDetach(&(PVOID&)Real_DeviceIoControl, (PVOID)Mine_DeviceIoControl);
