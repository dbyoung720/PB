...
#pragma pack(pop)
	...
	bool interlaced = false, top_field_first = false, repeat_field = false;
	if (m_fcc == '14VR') {
		::GetDimensions_X10(pDataIn, &width, &height, &interlaced, &top_field_first, &repeat_field);
	} else {
		::GetDimensions(pDataIn, &width, &height);
	}
	...
	hr = RVTransform(pDataIn, m_pI420, &transform_in, &transform_out, m_dwCookie);
...
HRESULT CRealVideoDecoder::CheckInputType(const CMediaType* mtIn)
{
	...
	if (!m_pInput->IsConnected()) {
		...
		if (ERROR_SUCCESS == key.Open(HKEY_CLASSES_ROOT, L"Software\\RealNetworks\\Preferences\\DT_Codecs", KEY_READ)
			...
			oldpath = buff;
			WCHAR c = oldpath[oldpath.GetLength()-1];
			if (c != L'\\' && c != L'/') {
				oldpath += L'\\';
			}
			key.Close();
		...
		if (ERROR_SUCCESS == key.Open(HKEY_CLASSES_ROOT, L"Helix\\HelixSDK\\10.0\\Preferences\\DT_Codecs", KEY_READ)
				&& ERROR_SUCCESS == key.QueryStringValue(nullptr, buff, &len) && wcslen(buff) > 0) {
			newpath = buff;
			WCHAR c = newpath[newpath.GetLength()-1];
			if (c != L'\\' && c != L'/') {
				newpath += L'\\';
			}
			key.Close();
...
HRESULT CRealAudioDecoder::CheckInputType(const CMediaType* mtIn)
{
	...
	if (!m_pInput->IsConnected()) {
		...
		WCHAR fourcc[5] = {
			(WCHAR)((mtIn->subtype.Data1>>0)&0xff),
			(WCHAR)((mtIn->subtype.Data1>>8)&0xff),
			(WCHAR)((mtIn->subtype.Data1>>16)&0xff),
			(WCHAR)((mtIn->subtype.Data1>>24)&0xff),
			0
		};

		if (!wcscmp(L"RACP", fourcc) || !wcscmp(L"\xff", fourcc)) {
			wcscpy_s(fourcc, L"RAAC");
		}
