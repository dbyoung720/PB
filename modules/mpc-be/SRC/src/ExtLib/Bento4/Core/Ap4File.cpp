            ...
            case AP4_ATOM_TYPE_SIDX:
                {
                    ...
                    if (it != sidxAtoms.end()) {
                        it->second->Append(sidxAtom);
                        delete atom;

                        sidxAtom = sidxAtoms[referenceId];
                    } else {
                        sidxAtoms.emplace(referenceId, sidxAtom);
                        m_OtherAtoms.Add(sidxAtom);
                    }

                    if (AP4_FAILED(stream.Seek(sidxAtom->GetSegmentEnd()))) {
                        bBreak = true;
                    }
